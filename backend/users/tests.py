...
class UserAccountTests(APITestCase):
    ...
    def test_user_registration(self):
        ...
        self.assertEqual(UserAccount.objects.get(email='jane.smith@example.com').first_name, 'Jane')
    ...
    def test_update_user(self):
        ...
        response = self.client.put(reverse('updateUser'), data=update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_user(self):
        self.client.login(username=self.user['email'], password=self.user['password'])
        response = self.client.post(reverse('deleteUser'), {'id': self.user_instance.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
